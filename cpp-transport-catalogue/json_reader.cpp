...
void JsonReader::ParseInput(std::istream& in) {
    ...
    auto& base_reqs_array = parsed_json_.at("base_requests"s).AsArray();
    ...
    for(auto& entry : base_reqs_array) {
        ...
        if(dict.at("type"s).AsString() == "Stop"s) {
            ...
            database_.AddStop(std::string(stop_name), stop_coords);
    ...
    for(const auto& [from_stop, distances] : stops_road_distances) {
        for(const auto& [to_stop, dist] : distances) {
            database_.SetRoadDistance(from_stop, to_stop, dist);
        }
    }
    ...
    for(auto& entry : base_reqs_array) {
        ...
        if(dict.at("type"s).AsString() == "Bus"s) {
            ...
            database_.AddBus(dict.at("name"s).AsString(), stops, dict.at("is_roundtrip"s).AsBool());
        }
    }
    //4.Parse and Apply Map Renderer Settings
    if(parsed_json_.count("render_settings") > 0) {
        auto& rs = parsed_json_.at("render_settings").AsMap();
        ...
        req_handler_.UploadRendererSettings(std::make_shared<RendererSettings>(std::move(settings)));
//    } else {
//        std::cerr << "renderer_settings not found in json" <<std::endl;
    }
    
    //5.Store Database Stat Requests
    if(parsed_json_.count("stat_requests") > 0) {
        for(auto& json_request : parsed_json_.at("stat_requests").AsArray()) {
            if(json_request.AsMap().empty()) {
                throw std::runtime_error("");
            }
            auto& request_dict = json_request.AsMap();
            
            stat_requests_.push({request_dict.at("type"s).AsString(), request_dict.at("name"s).AsString(),
                request_dict.at("id"s).AsInt()});
        }
//    } else {
//        std::cerr << "Stat_requests not found in json" <<std::endl;
    }
}
...
void JsonReader::ProcessStatRequests() {
    while(!stat_requests_.empty()) {
        StatRequest request = std::move(stat_requests_.front());
        stat_requests_.pop();
